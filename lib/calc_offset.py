...
def calc_offset(input_x, input_y, coord, input_off_x, input_off_y, offcoord, dcos, timestamp):
    ...
    on_coord = SkyCoord(x, y, frame=on_frame, location=nobeyama,obstime=timestamp)
    xsign = 0
    ysign = 0
    ...
    if off_frame == "altaz":
        ...
        ret_coord = SkyCoord(real_az, real_alt, frame=off_frame, unit="deg", location=nobeyama, obstime=timestamp)
        ...
        ret_coord = SkyCoord(real_lon, real_lat, frame=off_frame, unit="deg", location=nobeyama, obstime=timestamp)
        on_coord = ret_coord.transform_to(on_frame)
